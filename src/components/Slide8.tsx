  ...
  return (
    <div className={`p-6 sm:p-8 rounded-3xl shadow-2xl relative overflow-hidden w-full h-full flex flex-col ${isDark ? 'bg-[#1e1e1e] shadow-black/60 border border-[#2a2a2a]' : 'bg-white shadow-gray-300/60 border border-gray-100'}`}>
      ...
      <div className="text-center mb-4 sm:mb-6 shrink-0">
        <h2 className="text-3xl sm:text-4xl font-bold mb-2 flex items-center justify-center gap-3 text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-rose-600">
          <Flag className="text-red-500" size={32} fill="currentColor" /> Banderas Rojas (Red Flags)
        </h2>
        <p className={`text-sm sm:text-base max-w-3xl mx-auto ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
          Haz clic en cada bandera roja para descubrir la estrategia o acción recomendada para abordarla.
        </p>
      </div>
        ...
        {redFlags.map((flag) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* Front */}
                <div className={`absolute inset-0 [backface-visibility:hidden] rounded-2xl p-3 sm:p-4 flex flex-col items-center justify-center text-center border transition-colors ${isDark ? 'bg-[#2a2a2a] border-[#3a3a3a] group-hover:border-red-500/50' : 'bg-red-50 border-red-100 group-hover:border-red-300'}`}>
                  <Flag className="text-red-500 mb-2 shrink-0" size={24} />
                  <h3 className={`font-bold text-sm sm:text-base mb-1 sm:mb-2 leading-tight ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{flag.title}</h3>
                  <p className={`text-xs leading-snug ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{flag.desc}</p>
                </div>

                {/* Back */}
                <div className={`absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl p-3 sm:p-4 flex flex-col items-center justify-center text-center border ${isDark ? 'bg-gradient-to-br from-red-900/40 to-rose-900/40 border-red-500/30' : 'bg-gradient-to-br from-red-500 to-rose-500 border-red-600 text-white'}`}>
                  <h3 className={`font-bold text-sm sm:text-base mb-1 sm:mb-2 leading-tight ${isDark ? 'text-red-400' : 'text-white'}`}>{flag.actionTitle}</h3>
                  <p className={`text-xs leading-snug ${isDark ? 'text-gray-300' : 'text-red-50'}`}>{flag.actionDesc}</p>
                </div>
              </motion.div>
            </div>
          );
